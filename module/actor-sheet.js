...
class DCCActorSheet extends ActorSheet {
  ...
  async getData(options) {
    ...
    if (data.isNPC) {
      ...
      this.options.template =
        "systems/dcc/templates/actor-sheet-zero-level.html";
    ...
    data.system.config.rollAttackBonus =
      this.actor.getAttackBonusMode() === "manual";
    ...
    data.notesHTML = await TextEditor.enrichHTML(
      this.actor.system.details.notes.value,
      {
        async: true,
    ...
    if (this.actor.system.class?.corruption) {
      data.corruptionHTML = await TextEditor.enrichHTML(
  ...
  activateListeners(html) {
    ...
    if (this.actor.isOwner) {
      ...
      html
        ...
        .click(this._onRollAbilityCheck.bind(this));
      ...
      // Skills
      html
        .find(".skill-check.rollable")
        .click(this._onRollSkillCheck.bind(this));
      ...
      // Disapproval
      html
        .find(".disapproval-range")
        .click(this._onApplyDisapproval.bind(this));
      ...
      // Attack Bonus
      html
        .find(".attack-bonus.rollable")
        .click(this._onRollAttackBonus.bind(this));
  ...
  _onDragStart(event) {
    ...
    if (classes.contains("ability")) {
      ...
      // delete original weapon -- TODO/note this should really be drop-location specific... (override _onDrop or specify callback ?)
      // e.g. right now it is deleting the weapon when you drag it to macro bar, which isn't desired
      ...
      // delete original item --  TODO/note this should really be drop-location specific... (override _onDrop or specify callback ?)
      // this.actor.deleteEmbeddedDocuments("Item", [itemId]);
  ...
  _onRollAbilityCheck(event) {
    ...
    const rollUnder =
      ability === "lck" &&
      event.currentTarget.className !== "ability-modifiers";
  ...
  async _updateObject(event, formData) {
    ...
    if (event.currentTarget) {
      ...
      if (expanded.itemUpdates) {
        if (
          parentElement.classList.contains("weapon") ||
