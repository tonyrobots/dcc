...
Hooks.once("init", async function () {
  ...
  Actors.registerSheet("dcc", DCCSheets.DCCActorSheetInventory, {
    types: ["Player", "NPC"],
    label: "DCC.DCCActorSheetInventory",
  });
  ...
  Handlebars.registerHelper("dccPackExists", function (pack, options) {
    return new Handlebars.SafeString(
      game.packs.get(pack) ? options.fn(this) : options.inverse(this)
    );
  });
...
function checkReleaseNotes() {
  ...
  $(document).on("click", ".dcc-release-notes", () =>
    _onShowJournal("dcc.dcc-userguide", "DCC System Changelog")
  );
  $(document).on("click", ".dcc-credits", () =>
    _onShowJournal("dcc.dcc-userguide", "Credits")
  );
  $(document).on("click", ".dcc-user-guide", () =>
    _onShowURI(
      "https://github.com/foundryvtt-dcc/dcc/wiki/DCC-System-User-Guide"
    )
  );
}
...
function registerTables() {
  ...
  CONFIG.DCC.disapprovalPacks.addPack(
    game.settings.get("dcc", "disapprovalCompendium"),
    true
  );
  ...
  CONFIG.DCC.criticalHitPacks.addPack(
    game.settings.get("dcc", "critsCompendium"),
    true
  );
  ...
  try {
    ...
    if (rollTable) {
      ...
      if (results.roll.terms.length > 0) {
        ...
        if (naturalRoll === 1) {
          const fumbleResult = await rollTable.draw({
            roll: new Roll("1"),
            displayChat: false,
          });
          ...
          if (actor.type === "Player") {
            const critRoll =
              results.roll._total + actor.system.details.level.value;
            const critRollObject = new Roll(String(critRoll));
            const critResult = await rollTable.draw({
              roll: critRollObject,
              displayChat: false,
            });
      ...
      await game.dcc.SpellResult.addChatMessage(rollTable, results, {
        crit,
        fumble,
        itemId: item?.id,
      });
...
function _createDCCAbilityMacro(data, slot) {
  ...
  const macroData = {
    ...
    img: EntityImages.imageForMacro(
      abilityId,
      rollUnder ? "abilityRollUnder" : "ability"
    ),
  };

  // If this is a roll under check make it clear in the macro name
  if (rollUnder) {
    macroData.name = game.i18n.format("DCC.RollUnder", {
      name: macroData.name,
    });
  }
...
function _createDCCInitiativeMacro(data, slot) {
  ...
  return {
    ...
    command:
      "const _actor = game.dcc.getMacroActor(); if (_actor) { _actor.rollInitiative(token, game.dcc.getMacroOptions()) }",
...
function _createDCCHitDiceMacro(data, slot) {
  ...
  return {
    ...
    command:
      "const _actor = game.dcc.getMacroActor(); if (_actor) { _actor.rollHitDice(game.dcc.getMacroOptions()) }",
    img: EntityImages.imageForMacro(
      game.dcc.DiceChain.getPrimaryDie(data.data.dice),
      "hitDice"
    ),
  };
}
...
function _createDCCLuckDieMacro(data, slot) {
  ...
  return {
    ...
    command:
      "const _actor = game.dcc.getMacroActor(); if (_actor) { _actor.rollLuckDie(game.dcc.getMacroOptions()) }",
    img: EntityImages.imageForMacro(
      game.dcc.DiceChain.getPrimaryDie(die),
      "luckDie"
    ),
  };
}
...
function _createDCCSpellCheckMacro(data, slot) {
  ...
  const macroData = {
    ...
    command:
      "const _actor = game.dcc.getMacroActor(); if (_actor) { _actor.rollSpellCheck() }",
...
function _createDCCAttackBonusMacro(data, slot) {
  ...
  return {
    ...
    command:
      "const _actor = game.dcc.getMacroActor(); if (_actor) { _actor.rollAttackBonus(game.dcc.getMacroOptions()) }",
    img: EntityImages.imageForMacro(
      game.dcc.DiceChain.getPrimaryDie(die),
      "attackBonus"
    ),
...
function _createDCCWeaponMacro(data, slot) {
  ...
  const macroData = {
    name: item.name,
    command: `game.dcc.rollDCCWeaponMacro("${weaponSlot}", Object.assign(${JSON.stringify(
      options
    )}, game.dcc.getMacroOptions()));`,
    img: item.img,
  };
...
function _createDCCApplyDisapprovalMacro(data, slot) {
  ...
  return {
    ...
    command:
      "const _actor = game.dcc.getMacroActor(); if (_actor) { _actor.applyDisapproval() }",
...
function _createDCCRollDisapprovalMacro(data, slot) {
  ...
  return {
    ...
    command:
      "const _actor = game.dcc.getMacroActor(); if (_actor) { _actor.rollDisapproval() }",
...
function rollDCCWeaponMacro(itemId, options = {}) {
  ...
  if (!actor)
    return ui.notifications.warn(
      game.i18n.localize("DCC.MacroNoTokenSelected")
    );
